...
object Demo05_WindowCounting {
  def main(args: Array[String]): Unit = {
    ...
    val resultDStream: DStream[(String, Int)] = socketDStream.flatMap(_.split(" "))
      .map(_ -> 1)
//      .reduceByKey(_ + _) 因为reduceByKey是默认按照批次来聚合 也就是不理会窗口, 只按照批次玩
      // 也就是 窗口长度 == 微批处理间隔的时候, 用reduceByKey是合适的
      // reduceByKeyAndWindow 参数1: 聚合逻辑, 参数2是窗口长度 参数3 滑动距离
      .reduceByKeyAndWindow((_: Int) + (_: Int), Duration(5000L), Duration(5000L)) // 表明按照窗口进行聚合
